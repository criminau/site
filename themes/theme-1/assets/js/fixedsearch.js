...
// assets/js/fixedsearch.js
// assets/js/fuse.js
// hugo minify -> /js/fuse.min.js
/*--------------------------------------------------------------
...
fixedsearch = function(){
	...
	function search_exec(term) {
		...
		if (results.length > 0) {
			first = search_results.firstChild.firstElementChild; // first result container â€” used for checking against keyboard up/down location
			last = search_results.lastChild.firstElementChild; // last result container â€” used for checking against keyboard up/down location
		}
	}
}();
